...
**Open item, owned by Claude on CT105, not by Tom:** a connector gate hook (`confirm-connector.sh`) has been written, with per-server `PreToolUse` matchers covering all three connectors. It is **not active yet.** Hook config edits do not take effect inside an already-running session, so activation needs a session restart or an explicit review first. Until that happens a Gmail send or trash issued as an MCP call still runs unprompted. Activating it is Claude's to do on CT105; Tom cannot reach that machine and has not attempted it.
