...
**✅ Correction, verified 2026-08-15 (supersedes the struck block below): "Claude" above is still two surfaces, but the split is not the one previously documented.** Gmail, Calendar and Drive all have working Claude Code tools, exposed as MCP tools named `mcp__claude_ai_Gmail__*`, `mcp__claude_ai_Google_Calendar__*` and `mcp__claude_ai_Google_Drive__*`. The claude.ai Settings→Connectors OAuth grant propagated to the Claude Code surface with no separate OAuth flow. Evidence from real tool calls in Claude's CT105 session: `list_calendars` returned `secureprospective@gmail.com` plus "Holidays in United States", timezone `America/Chicago`; `list_labels` returned INBOX 963 messages / 99 unread, SENT 164, DRAFT 1, TRASH 90.

**Practical effect on this diagram: the Gmail and Calendar edges are no longer forced into a claude.ai chat step.** They can be driven from Claude Code. The seam did not disappear, it moved: the Worker/D1 layer is Tom's territory end-to-end, and the connector layer is Claude's authenticated session on CT105. Tom's session is not authenticated against these connectors and cannot exercise them. Keep that seam explicit when designing a workflow, and name which session owns each edge.

**New risk to design against: the Gmail tool surface is not draft-only.** It includes `send_message`, `reply`, `forward`, `trash_message`, `trash_thread` and `mark_message_spam`, with no vendor-side guardrail. SP's own draft-only rule (`GOOGLE_WORKSPACE_PILOT.md`, Phase 0 step 4) is now the only thing enforcing draft-only behavior on these edges.

**Open item, owned by Claude on CT105, not by Tom:** CT105's `PreToolUse` hook has matcher `"Bash"` only, so it does not gate MCP tool calls. Any workflow in this architecture that reaches Gmail through Claude Code is currently ungated at the harness level. Closing that is Claude's to do on CT105, which Tom cannot reach.
