...
payload: "localhost/sp-plus-kde:t29"
...
figures:
  - value: "80"
    label: "Runtime assertions measured on a booted machine, all passing on t29."
  - value: "74"
    label: "Build gates embedded in the Containerfile. One failure fails the build."
  - value: "14"
    label: "Of our own checks caught measuring nothing, published in the register."
  - value: "12"
    label: "Residual risks that remain after every control listed."
start:
  - id: "4-the-evidence-system"
    n: "4"
    title: "The evidence system"
    note: "How every claim in this document is produced, and the rule that makes a gate mean something."
  - id: "7-failure-register-checks-that-passed-while-measuring-nothing"
    n: "7"
    title: "Failure register"
    note: "Fourteen of this project's own checks that reported green while measuring nothing."
  - id: "9-residual-risk"
    n: "9"
    title: "Residual risk"
    note: "What remains true after all 80 assertions pass. Press hardest on items 3 and 8."
---

## 0. How to read this, and how to check it

This is not a marketing document and it is not a compliance statement. SecureProspective
...
rather than requiring an unlock, and why `sudo` is passwordless. Each is explained in place
...
| The SP+ cosign private key | SecureProspective, held in `~/.config/sp-plus-signing/` on the build host | Never leaves that directory. The public half ships in the image |
| The build host | SecureProspective | One machine, no CI |
...
custom Secure Boot keys and requires no MOK enrolment**, because an advisor cannot be walked through
...
matters, because a service that is not running but can be D-Bus activated is a service that is
running on demand.
...
refused. That second assertion is the discriminating one, because a check that only rejects
...
| 853 reachable | 5 of 5 | 5 of 5 | 0.40 s | n/a | 200 |
...
| Layer | Path | Reaches |
|---|---|---|
| pam_env | `/etc/environment` | tty logins and `su`. **Not ssh**, because Fedora does not stack `pam_env` in `sshd` or `password-auth` |
...
- **Crash reports open the advisor's own mail client** to a SecureProspective address rather
...
| 2026-09-10 09:00 to 11:00 | alpha4 built; posture measured against the research rubric for the first time |
...
| open | Should the published ghcr tag be re-signed in the readable format, activating end-to-end verification? |
