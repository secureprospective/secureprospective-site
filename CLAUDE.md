...
- **On resume:** check the `.DONE` marker first. If done, `ssh beelink "cat ~/work/secureprospective-site/docs/research/page-transition-prototype-notes.md"` for DeepSeek's own report (what it built, what it's unsure of), then review its actual diff (`ssh beelink "cd ~/work/secureprospective-site && git log session/page-transition-prototype -3 --stat"`), pull the branch to CT105 the same way bird/other Beelink branches come home (`git remote add beelink ssh://chris@beelink~/work/secureprospective-site` if not already added, `git fetch beelink session/page-transition-prototype`), then **treat it as a lead, not a merge candidate** — DeepSeek is the routine-edit lane, not the design-judgment owner; review its choreography/code quality against the actual brief before adopting any of it, expect to rewrite rather than rubber-stamp.
...
## SP+ ISO location — standing order
