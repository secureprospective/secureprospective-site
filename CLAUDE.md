...
- `functions/api/ecosystem/knowledge-graph.ts` + `functions/api/ecosystem/mcp/router.ts` — built and verified end-to-end against a local D1 copy via `wrangler pages dev` (seeded real entities, exercised every route, confirmed the MCP tool chain genuinely works — pricing_lookup/service_catalog/etc. return real graph-traversal results). Auth fails closed on both (401 with or without a JWT) because no Cloudflare Access application exists on this account yet ("Access: None" in the Cloudflare Configuration section above) — there's no real `AccessJwtValidator` to check against. **These were meant to stay held (local-commit-only, `cbaeddf`) until Access was configured — a routine `git push` at session close accidentally pushed them to the branch anyway (2026-07-24).** Christopher's call afterward: leave it, no real data is exposed (remote D1 is empty, writes/MCP fail closed regardless), it's still only a preview URL on a session branch, not `main`. **The one thing this did expose:** `knowledge-graph.ts` GET (read) endpoints are intentionally open by design (no auth on reads, matching the MCP tools' public-read intent) — so reads are live on the preview URL right now, just against an empty remote D1. **Still do not merge to `main`** until Access is configured — that's step 4, still not done.
