...
**Beelink routine-edit lane (added 2026-08-05) — RETIRED 2026-09-02.** OpenCode and the DeepSeek lane were retired altogether; the `.opencode/` skill install has been removed from this repo. Do not dispatch through `opencode run`. Mechanical execution goes to the other fleet agents. The description below is kept for history only:

 a second clone lives on Beelink at `~/work/secureprospective-site`, wired with Impeccable for both Claude Code (`.claude/skills/impeccable`) and OpenCode (`.opencode/skills/impeccable`) — installed via `npx impeccable install` with harnesses `claude,opencode`. Purpose: routine/mechanical edits (design-system documentation drift, type-ramp/color fixups, small targeted Impeccable commands) run there on **DeepSeek free** (`opencode/deepseek-v4-flash-free`, opencode.ai's native free tier, no proxy needed — this is a different arrangement from Hermes's zen-noauth-proxy workaround) instead of burning Claude's tokens on mechanical work. Invoke: interactively via `opencode` (model picker) or one-shot via `opencode run --model opencode/deepseek-v4-flash-free "<task>"`, both from `~/work/secureprospective-site`. CT105 fetches the same way as bird: `git remote add beelink ssh://chris@beelink/home/chris/work/secureprospective-site && git fetch beelink <branch>` (SSH alias `beelink` added to `/root/.ssh/config`, key `/root/.ssh/beelink`). Claude still owns: design *judgment* (critique synthesis, live-mode direction planning, redesign calls), merge review, and the actual Cloudflare deploy — this lane is execution-only, same split as [[local_model_teammate_doctrine]]. First real output: DESIGN.md's Micro-UI Text Grays subsection (commit `93674ab`), closing a P3 finding from this session's critique.
...
**Resume instructions for this task (SUPERSEDED 2026-09-02 — OpenCode/DeepSeek retired; the prototype output already sits in `docs/research/page-transition-prototype-notes.md`, so treat the steps below as history, not as a dispatch procedure):**
- Branch: `session/page-transition-prototype` on the **Beelink clone** (`ssh beelink`, path `~/work/secureprospective-site`), based on `main` @ `bb85798`. Nothing on CT105 touches this branch yet.
...
- Check completion: `ssh beelink "ls ~/work/secureprospective-site/docs/research/page-transition-prototype.DONE"` — exists once done. If missing, check `ssh beelink "tail -40 ~/work/secureprospective-site/docs/research/page-transition-prototype.log"` for progress/errors.
- Full prompt sent to DeepSeek (for context on exactly what was asked): `~/work/secureprospective-site/docs/research/_transition_prompt.md` on Beelink (also was written to `/tmp/claude-0/.../scratchpad/deepseek-transition-prompt.md` on CT105 this session, may not survive to next session — the Beelink copy is the durable one).
- **On resume:** check the `.DONE` marker first. If done, `ssh beelink "cat ~/work/secureprospective-site/docs/research/page-transition-prototype-notes.md"` for DeepSeek's own report (what it built, what it's unsure of), then review its actual diff (`ssh beelink "cd ~/work/secureprospective-site && git log session/page-transition-prototype -3 --stat"`), pull the branch to CT105 the same way bird/other Beelink branches come home (`git remote add beelink ssh://chris@beelink/home/chris/work/secureprospective-site` if not already added, `git fetch beelink session/page-transition-prototype`), then **treat it as a lead, not a merge candidate** — DeepSeek is the routine-edit lane, not the design-judgment owner; review its choreography/code quality against the actual brief before adopting any of it, expect to rewrite rather than rubber-stamp.
...
- [ ] **Google Workspace pilot — connectors LIVE, safety harness ARMED (2026-08-15), Phase 2 still unrun.** SP is the pilot business for wiring Claude's official Gmail/Calendar/Drive connectors into real day-to-day business use (ClaudeBox as head brain, chat-mode only, never Cowork). Full build map + staged test sequence: `docs/GOOGLE_WORKSPACE_PILOT.md`. Governed by the adopted portability rule (memory `feedback_portability_rule_business_builds`) — SOPs must land in portable markdown, not Claude-only automation. **Status:** connectors verified working as Claude Code MCP tools; read/search/draft unblocked; 8 destructive tools hard-denied and 6 on `ask` via CT105 `permissions` config, verified live (real send test `1a006b5214d7ebbc`, receipt confirmed). **Read the "Enforced gates" section of the pilot doc before any connector work** — in particular, `ask` does NOT prompt in a child/auto-mode session (it auto-approves and writes an `allow` entry that outranks the rule), so connector work belongs in a plain interactive session. Memory: `lesson_ask_permission_bypassed_in_child_sessions`. **Next real gate is Phase 2**, the 9-step staged sequence, still unrun. **Next build:** contacts list organized by use case and groups — decide first whether contacts live as Gmail labels, a Drive file, or the consulting-side store in `NO_CRM_ARCHITECTURE.md` §E.
- [ ] **"Never need a CRM" architecture — SCOPE LOCKED 2026-08-15, not started.** No CRM SaaS specifically, not no-cloud — Cloudflare + Google Workspace stay as infrastructure. v1: Workspace connectors + minimal D1 schema (contacts/deals/activities/**event_log from day one**, everything else retrofittable) + Sheets where needed + a simple dashboard. Applies across the whole business, IMO included — compliance gets engineered in over time, not exempted or blocking. Full doc + panel findings: `docs/NO_CRM_ARCHITECTURE.md`. Next concrete step: Google Workspace pilot Phase 0/1 (`docs/GOOGLE_WORKSPACE_PILOT.md`) — the connector enablement itself is a Claude.ai Settings action only Christopher can do; the D1/dashboard scaffolding can be built in Claude Code in parallel.
