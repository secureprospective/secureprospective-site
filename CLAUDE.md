...
**Still open from that critique — three P2s, one P3, not yet started:**
1. **[P2] `.teaser-meta` fails WCAG AA contrast.** `ink` text at 60% opacity over white composites to ~4.27:1 at 11px, below the 4.5:1 AA threshold for small text. Fix: swap the opacity trick for the already-named `#666` "Fineprint Gray" token (DESIGN.md's Micro-UI Text Grays section already defines this token for exactly this role). Location: `src/pages/index.astro`, `.teaser-meta` rule (~line 189-197 as of the critique run, may have shifted).
2. **[P2] Two competing yellow primary CTAs collapse DESIGN.md's own hierarchy rule.** "See the method" (hero) and "Start with a conversation" (loop-closer) are both full `.btn` Ticker Yellow; DESIGN.md's Buttons section says yellow signals *the* conversion action, once per page. This exact conflict was already logged as an open item back in the 2026-08-05 session (see below) and still hasn't been resolved either direction. Pick one: demote "See the method" to `.btn--ink` (it opens the MethodWidget panel, not the booking funnel), or formally amend the DESIGN.md rule to match what's shipped.
3. **[P2] MethodWidget accessibility drifted from its sibling ChatWidget.** `ChatWidget.astro` moves focus into the panel on open and returns it to the trigger on close; `MethodWidget.astro` does neither. Separately, `MethodWidget`'s trigger `<a>` unconditionally calls `preventDefault()` on click, which also intercepts ctrl/cmd-click, silently breaking "open in new tab." Fix: mirror ChatWidget's focus handling, gate the `preventDefault()` on `!e.metaKey && !e.ctrlKey && e.button === 0`.
4. **[P3] Chat lead-gate charges an email before the first answer**, which cuts against PRODUCT.md's own framing of the chatbot as "secondary self-qualification, not the primary funnel." Lower urgency, worth a real test (does lead volume actually drop if one question is allowed ungated?) before assuming the current wall is correctly calibrated.

Read the full critique snapshot (`.impeccable/critique/2026-08-06T02-07-21Z__src-pages-index-astro.md`) before starting any of these — it has the complete heuristics table, persona red flags, and minor observations (footer tagline tone, DESIGN.md's stale Card-Lift shadow-color swatch, the missing 12px type-ramp step) that didn't make it into this summary.
...
**2026-08-06 full session (logo/hero-motion + page-transition + buttons + motif rework + critique) is DONE and merged to `main` (`5dc4d02`, `dc05750`, live, pushed).** See Current Build State above for full detail. Impeccable live-mode's multi-variant-pick workflow is proven across several feature passes now; its click-to-select picker does NOT work well on `pointer-events:none` canvas layers (the overlay eats the click) — for that case, build real switchable variants behind a `?param=` query string instead and let Christopher flip between them directly, no picker needed. The Beelink DeepSeek lane is proven for web-research, small reference-prototype builds, and mechanical edits.

Immediate next-session priorities, in order (all four are from the 2026-08-06 `/impeccable critique` run, snapshot at `.impeccable/critique/2026-08-06T02-07-21Z__src-pages-index-astro.md`, full detail in Current Build State above):
1. **[P2] `.teaser-meta` contrast fix** — swap the `opacity: 0.6` trick for the `#666` Fineprint Gray token. Quick, isolated.
2. **[P2] Two-yellow-CTA hierarchy conflict** — decide: demote "See the method" to `.btn--ink`, or amend DESIGN.md's one-primary-CTA rule to match reality. This is the SAME open conflict as item 4 below (Reconcile DESIGN.md's stale one-primary-CTA rule) — one decision closes both.
3. **[P2] MethodWidget accessibility** — mirror ChatWidget's focus-in/focus-out handling, gate the trigger's `preventDefault()` on modifier keys.
4. **[P3] Chat lead-gate email wall** — lower urgency, worth testing rather than just changing.
