...
**Character:** Primal is a heavy, geometric, uppercase-by-default display face — it reads as stamped card-title lettering. IBM Plex Sans carries the technical, engineered-not-generic body voice without competing with Primal for attention.
...
The system is flat by default — no ambient soft shadows, no blur-based depth. Where depth is needed, it comes from a **hard-edge offset shadow**: zero blur, a flat directional offset, reading as a card physically lifted off the table rather than a glow. This was previously locked as "no drop shadows at all" in an earlier pass, but that blanket rule wasn't grounded in anything — the property-card metaphor actually wants this kind of shadow, used deliberately on primary interactive/lifted elements (currently the chat launcher; available to extend to other lifted card-moments as the redesign progresses).
...
- **Primary:** Ticker Yellow background, Ledger Ink text, 16px/32px padding, uppercase Primal label
- **Primary hover:** shifts to Vault Gold
- **Ink variant:** Ledger Ink background, white text, 2px Ledger Ink border
...
- **Shadow Strategy:** flat by default; card-lift shadow reserved for elements that should read as physically raised (see Elevation & Depth)
