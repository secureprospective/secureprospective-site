#!/usr/bin/env python3
"""Narrow Advisor OS POC RPC boundary and local PWA server.
...
ROOT = Path(os.environ.get("ADVISOR_OS_ROOT", Path(__file__).resolve().parents[1]))
PWA_ROOT = ROOT / "pwa"
...
STATE_DIR = Path(os.environ.get("ADVISOR_STATE_DIR", "/var/lib/advisor-os"))
STATE_PATH = STATE_DIR / "printer-fixture.json"
EVENTS_PATH = STATE_DIR / "events.jsonl"
AI_ENDPOINT = os.environ.get("ADVISOR_AI_ENDPOINT", "").strip()
FIXTURE = os.environ.get("ADVISOR_PRINTER_FIXTURE", "1").lower() not in {"0", "false", "no"}
...
def rpc(method: str, params: dict[str, Any]) -> dict[str, Any]:
    if method == "health":
        return {"ok": True, "service": "advisor-os-rpc", "fixture": FIXTURE}
...
def main() -> None:
    host = os.environ.get("ADVISOR_LISTEN_ADDRESS", "127.0.0.1")
    port = int(os.environ.get("ADVISOR_LISTEN_PORT", "8765"))
    ensure_state()
