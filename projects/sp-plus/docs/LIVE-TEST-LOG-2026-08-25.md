...
Date: 2026-08-25
Artifact: `artifacts/iso/sp-plus-installer.iso`
...
**Defect found:** `interactive-defaults.ks` used `registry:localhost/sp-plus:poc`, but image-builder embeds the payload under `/var/lib/containers/storage` and the ISO storage manifest contains the image as `localhost/sp-plus:poc`. The installer therefore could not use the embedded payload as written.

**Change:** Use `containers-storage:localhost/sp-plus:poc`. Also enable DHCP in the ISO boot arguments, add a Kickstart DHCP network line, and provide an explicit Fedora 43 repository URL so Anaconda does not depend on the failing local-media/mirror default.
...
- The installed Fedora 43 system requested the LUKS passphrase `advisor-poc`, reached multi-user.target, started `sp-plus.service`, and accepted `advisor` / `advisor-poc` login on serial console.
