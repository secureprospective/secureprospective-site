...
| D49 | **`hardened_malloc` is dropped from doc 15. SP+ ships `glibc.malloc.tcache_count=0` instead, and says plainly that it is smaller** | Taken 2026-09-11 on Christopher's explicit delegation ("give me your recommendation"), after measurement. `hardened_malloc` is in none of the four repositories SP+ ships, so it needs a fifth trust root; Secureblue's own tracker says the config that enables the allocator is not in their RPM; and the documented breakage class is Electron, which here is Brave, Zoom and Signal. The in-repo alternative was measured: `glibc.malloc.check` and `glibc.malloc.perturb` are **inert** on glibc 2.43 because their implementation moved to `libc_malloc_debug.so`, which is not in the image, and `ld.so --list-tunables` still reports check as set. Only `tcache_count=0` is real. It removes tcache poisoning as a primitive and gives none of hardened_malloc's guard pages, randomised allocation, isolated size classes or delayed reuse — and the Security Evidence Report must say exactly that. Document 15 §4a, ledger `PHASE-T25-2026-09-11-hardened-malloc.md` | Reopens if SP+ decides to vendor and build the allocator from source, adding no repository |
| D50 | **SELinux confinement for Brave (T2.1) is deferred to a later branch. The permissive domain is not shipped** | Taken 2026-09-11 on the same delegation. The mechanism is proven: a CIL module loads without reference-policy headers, Brave transitions into `brave_t`, and Brave keeps working there. The blocker is a number — 2,163 AVC denials across 145 distinct triples from **three headless page loads**. An enforcing domain built from that baseline breaks the advisor's day, which D44 forbids outright. A permissive domain blocks nothing and, since SP+ cannot collect the logs it produces, would put telemetry in the scorecard under a heading that says hardening. The test VM cannot produce a real baseline because Brave will not load network pages inside it. **The prerequisite for reopening is about an hour of ordinary Brave use on real hardware under the permissive domain, keeping the audit log.** The working CIL skeleton is preserved in ledger `PHASE-T21-2026-09-11-brave-selinux.md` so nothing is rediscovered | Reopens the moment that audit log exists |
