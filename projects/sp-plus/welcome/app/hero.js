...
(function(){
  var phase=0, observer=null, canvas=null, onResize=null;
  ...
  function stop(){}
  function start(){if(!canvas)return;size();var c=canvas.getContext('2d');if(c)draw(c);}
  function boot(){canvas=document.getElementById('hero-fx-canvas');if(!canvas)return;onResize=function(){if(canvas&&canvas.offsetWidth>0)start();};window.addEventListener('resize',onResize);document.addEventListener('visibilitychange',function(){if(!document.hidden&&canvas&&canvas.offsetWidth>0)start();});observer=new IntersectionObserver(function(entries){entries.forEach(function(entry){if(entry.isIntersecting&&canvas&&canvas.offsetWidth>0)start();});},{threshold:.05});observer.observe(canvas);if(canvas.offsetWidth>0)start();}
